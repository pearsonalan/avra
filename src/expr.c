...
int get_symbol(struct prog_info *pi, char *label_name, int *data)
{
	...
	if (get_constant(pi,label_name,data))
		return(True);
	if (get_variable(pi,label_name,data))
		return(True);
	...
	if (get_label(pi,label_name,data))
		return(True);
	return(False);
}
	...
	for (i = 0;; i++)
	{
		...
		else
		{
			...
			if (isdigit(data[i]))
			{
				...
				{
					i += 2;
					while (isxdigit(data[i + length]))
						length++;                                                   // TODO: Sjekk overflow
