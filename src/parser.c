...
int parse_line(struct prog_info *pi, char *line)
{
	...
	while (IS_HOR_SPACE(*line))
		line++;                                                 /* At first remove leading spaces / tabs */
...
//		strcpy(pi->list_line, line);
	...
	for (i = 0; IS_LABEL(pi->fi->scratch[i]) || (pi->fi->scratch[i] == ':'); i++)
		if (pi->fi->scratch[i] == ':')          /* it is a label */
		{
			...
			i++;
			while (IS_HOR_SPACE(pi->fi->scratch[i]) && !IS_END_OR_COMMENT(pi->fi->scratch[i]))
				i++;
...
//			while(!IS_END_OR_COMMENT(data[i])) i++;     Problems with 2. operand == ';'
		...
		break;
	case TERM_SPACE:
		while (!IS_HOR_SPACE(data[i]) && !IS_END_OR_COMMENT(data[i]))
			i++;
		break;
	case TERM_DASH:
		while ((data[i] != '-') && !IS_END_OR_COMMENT(data[i]))
			i++;
		break;
	case TERM_COLON:
		while ((data[i] != ':') && !IS_ENDLINE(data[i]))
			i++;
		break;
	...
	{
		data[i--] = '\0';
		while (IS_HOR_SPACE(data[i]))
			data[i--] = '\0';
		return(0);
	}
	j = i - 1;
	while (IS_HOR_SPACE(data[j]))
		data[j--] = '\0';
	data[i++] = '\0';
	while (IS_HOR_SPACE(data[i]) && !IS_END_OR_COMMENT(data[i]))
		i++;
